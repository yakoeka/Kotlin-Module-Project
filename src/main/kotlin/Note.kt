class Note(val name: String, val textNote: String, private val archive: Archive, private val updateScreenCallback: ScreenUpdateCallback): MenuHolder {
    private val core = Core()
    override var menu: MutableList<MenuAction> = mutableListOf(
        MenuAction("Открыть заметку") { readNote()
                                      true},
        MenuAction("Назад") { back()
        true})
